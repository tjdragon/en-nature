...
- For InKind partner inquiries, include "InKind partner" in the query.
...
def use_case_exists(file_path):
    return os.path.isfile(file_path)

def save_use_case(file_path, use_case, response):
    try:
        with open(file_path, 'w') as file:
            file.write(f"{use_case}\n\n{response}\n")
        logger.info(f"Successfully saved strings to {file_path}")
    except Exception as e:
        logger.error(f"Error saving to file {file_path}: {str(e)}")
        logger.error(traceback.format_exc())

def read_use_cases(file_path):
    try:
...
use_cases = read_use_cases("ucs.txt")
...
for i, use_case in enumerate(use_cases, 1):
    ...
    try:
        if use_case_exists(f"{i}_use_case.txt"):
            print("Response already generated for this use case.")
            continue    
                                ...
                                if function_call.name == "search_places_with_details":
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            print(f"[G1]-> I found some restaurant information but had trouble formatting it. Here's what I found:\n{search_result}")
                                            continue
                                    
                                    if hasattr(response, 'text') and response.text:
                                        print("[G2]->", response.text)
                                        save_use_case(f"{i}_use_case.txt", use_case, response.text)
                                        ...
                                        print("[G4]-> I found some restaurant options for you. Let me know if you'd like more specific details.")
                                        logger.warning("No text response found after function execution")
        
        if not has_function_call:
            if hasattr(response, 'text') and response.text:
                print("[G5]->", response.text)
                save_use_case(f"{i}_use_case.txt", use_case, response.text)
                ...
                print("[G7]-> I'm processing your request. Could you please provide more details?")
