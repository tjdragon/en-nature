...
def handle_maps_search(query, max_results=20):
    logger.info(f"Handling Maps search for query: '{query}', max_results: {max_results}")
    formatted_results, raw_results = search_places_with_details(query, max_results)
    logger.info(f"Maps search returned {len(raw_results)} results")
